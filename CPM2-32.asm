...
; This copy of CP/M 2.2 is based on CPM22.Z80, which was itself reconstructed
; from a CP/M memory image in 1981 by Clark Calkins and then later translated
; into Z80 mnemonics. It originally contained the CCP and BDOS components of
; CP/M, plus a skeleton of the BIOS jump table at the end.
; An error was corrected(SBC A,M was replaced with SBC (HL) in 4 places) and
; the file extension was changed from .Z80 to .asm so that it would assemble
; correctly with ZASM.
; The SETIOB and GETIOB routines have been modified so that the value of the
; IOBYTE at location 0003 is ignored and is always effectively zero.
...
;
CBASE:
	JP	 COMMAND	;execute the command processor (CCP).
...
GETSETUC: LD	C,32
	JP	ENTRY
;
;   Routine to set the current drive byte at (CDISK).
;
SETCDRV:CALL	GETUSR		;get user number
	...
	RET	
;
;   Move currently active drive down to (CDISK).
;
MOVECD:	LD	A,(CDRIVE)
	LD	(CDISK),A
	RET	
...
;
	...
	CALL	MOVECD		;move current drive into place (CDISK).
...
;---------------------------------------------------------------
; GETIOB - Function to return the I/O byte.
;	This BDOS call has been modified to always return 00 as the
;	IOBYTE value. This causes the CONOLE, READER, LIST and
;	PUNCH devices to always be the TTY.
;---------------------------------------------------------------
GETIOB:
	LD	A,00		;2 bytes
	NOP				;1 byte
...
;---------------------------------------------------------------
; SETIOB - Function to set the I/O byte.
;	This BDOS call has been modified to effectively keep the
;	IOBYTE value at 00. This causes the CONOLE, READER, LIST and
;	PUNCH devices to always be the TTY.
;---------------------------------------------------------------
SETIOB:
	LD	 A,00			;2 bytes
...
;=============================================================================
; Disk parameter tables for a two-drive system with two logical 8" SSSD
; floppy disks. The 1st 3 tracks on disk 0 are reserved for the CP/M system code.
...
unallocated:
		DW	7777H,7777H,7777H,7777H,7777H,7777H,7777H,7777H
		DW	7777H,7777H,7777H,7777H,7777H,7777H,7777H,7777H
		DW	7777H,7777H,7777H,7777H,7777H,7777H,7777H,7777H
		DW	7777H,7777H,7777H,7777H,7777H,7777H,7777H,7777H
		DW	7777H,7777H,7777H,7777H,7777H,7777H,7777H,7777H
